...
- **Single-merchant chat** answers questions like *"what's my post-RTO ROAS by Meta campaign last 7d"* with cited per-row provenance in roughly 3.5–5 seconds end-to-end against real Gemini (measured: **p50 3.49s / p95 4.91s** across 40 real-mode requests in `scripts/bench_chat_latency.py`). That's the headline path and it's solid. The earlier ReAct loop sat at ~18s p95; the rewrite to the Plan → Execute → Join → Compose orchestrator is what closed the gap.
...
- **Citation contract** is the load-bearing claim, and it holds in CI: all 12 golden prompts pass, all 10 red-team prompts pass. Refusal/clarify cases short-circuit before the executor (planner emits `intent=refuse` or `intent=clarify` with zero tasks); answer-path red-team cases pass because the composer's text never leaks an uncited literal — and when it would, the verifier emits a `warning` SSE event alongside the answer rather than blocking it. There's no test path where a literal numeral reaches the user without either a footnote or a warning event.
...
- **LLM brittleness, told honestly.** In informal testing the planner picks the *wrong* metric on roughly 3-5% of real Gemini calls — usually gross-vs-net confusion or the wrong time grain. The joiner catches some of these (asks for a replan when results clearly don't fit the question), but not all. The fix over time is more curated `(question, plan)` few-shot pairs piped into the planner prompt. The eval gate is *"no uncited numerals,"* not *"answer is correct"* — a wrong-but-cited answer is structurally better than a hallucination (the founder can trace the citation back and discover the mistake), but it still ships.
...
**246 pytest tests, all green** (canonical number is whatever `uv run pytest -q` actually prints). The breakdown below is approximate but the shape is real. The chokepoint of the system (orchestrator pipeline + verifier + eval harness) gets the most coverage, because that's where a regression hurts most.
...
- 6 orchestrator (plan → execute → join → compose end-to-end, including replan and refusal short-circuit)
- 3 chat API route (POST /chat JSON + POST /chat/stream SSE event-sequence assertion)
- 4 Agent base + jsonb round-trip
- 15 RTO Risk Flagger
- 10 Meta Pauser
- 7 Pincode COD Blocker
- 27 eval harness (12 golden + 10 red-team + 5 static coverage / shape-validation)

The eval-harness count is the bar for citation honesty, and the construction matters. Every red-team prompt is encoded as a `(Plan, JoinerDecision, compose_chunks)` triple — refusal cases use `intent=refuse` so the planner short-circuits before any tool runs; answer-path cases script the composer to attempt the leak and assert either zero uncited numerals in the final text OR a `warning` event alongside footnotes. A static `test_*_yml_shape_is_orchestrator_compatible` validates every YAML entry against the live `Plan` / `JoinerDecision` Pydantic schemas at collection time, catching contract drift without needing a DB.
